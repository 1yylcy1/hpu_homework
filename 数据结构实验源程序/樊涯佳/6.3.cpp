...
void FindInDegree(ALGraph G) {//逆邻接表
	int i, count;
...
void path_exist(ALGraph G,int a,int b,int& t,int& k) {
	t++;
	int w,mid=0;
	ArcNode* p;
	visited[a] = true;
...
int main() {
	...
	FindInDegree(G);
	for (int i = 0; i < G.vexnum; ++i) {
		cout << G.vertices[i].data << "的入度为：" << indegree[i] << endl;
	}
	cout << endl;
	FindInDegree1(G);
	for (int i = 0; i < G.vexnum; ++i) {
	...
	cout << "请输入待判断是否存在简单路径的两点（以空格隔开）：";
	cin >> v0 >> v1;
	cout << "请输入简单路径的长度：";
	cin >> mid;
	path_exist(G, LocateVex(G, v0), LocateVex(G, v1),t,k);
	if (k == 1&&t==mid) {
		cout << "存在" << endl;
	}
	else
