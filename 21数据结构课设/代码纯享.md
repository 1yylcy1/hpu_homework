...
//3.创建邻接矩阵和边集数组,pn为边集结构体数组指针,0和99的都不用输入
void Solution ::create_adj_matrix(Edge*pn,int edge,int node)
{
    ...
    do
    {
        ...
        cin>>pn[k].start_vex>>pn[k].stop_vex>>pn[k].weight;//输入起点，终点，权重
        adj_matrix[pn[k].start_vex][pn[k].stop_vex]=pn[k].weight;//将输入的权重赋值给邻接矩阵
		adj_matrix[pn[k].stop_vex][pn[k].start_vex]=pn[k].weight;
        if(pn[k].weight==0) break;//如果输入的权重为0，则停止
		k++;
    }while(true);
...
int main()
{
	...
	case 1:
		...
		break;
	case 2:
		std::cout<<"请输入小区个数(顶点数)"<<endl;cin>>n;
		std::cout<<"请输入小区之间的路径数(边数)"<<endl;cin>>edge_num;
		//初始化邻接矩阵和边集数组
		A.initial_adj_matrix(edge_prim,n,edge_num);
		//创建邻接矩阵和边集数组
		A.create_adj_matrix(edge_prim,edge_num,n);
		A.adj_KrusKal(edge_prim,A.adj_matrix,edge_num);	
	case 3://测试KrusKal算法
